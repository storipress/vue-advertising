...
const advertising = shallowRef<Advertising | null>(
  new Advertising(config, plugins, onError)
);
const activate = computed(() =>
  advertising.value?.activate.bind(advertising.value)
);
...
useHead({
  script: [
    ...
    isPrebid
      ? {
          async: true,
          src: "https://acdn.adnxs.com/prebid/not-for-prod/1/prebid.js",
        }
      : {},
  ],
});
